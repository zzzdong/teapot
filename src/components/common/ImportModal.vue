<template>
  <n-modal v-model:show="showModal" preset="card" :style="{ width: '600px' }" title="Import">
    <!-- 步骤指示器 -->
    ...
    <template v-if="currentStep === 1">
      <div class="step-content">
        <n-tabs v-model:value="currentImportType" type="line" class="import-tabs">
          <n-tab-pane name="file" tab="File">
            <div class="tab-content">
<div class="import-drop-area" @click="handleImportFile" @drop.prevent="handleDrop" @dragover.prevent>
              <div class="drop-icon">
          ...
          <n-tab-pane name="text" tab="Text">
            <div class="tab-content">
              <div class="text-editor-wrapper">
                <MonacoEditor v-model:value="textContent" language="text" :min-lines="15" :max-lines="15" />
              </div>
            </div>
          </n-tab-pane>
        </n-tabs>
        ...
        <n-space justify="end" style="margin-top: 20px;">
          ...
          <n-button type="primary" :disabled="!canProceedToPreview" @click="handleNextStep">
            Next
          </n-button>
        </n-space>
      </div>
    ...
    <template v-else-if="currentStep === 2">
      <div class="step-content">
        <div class="preview-container">
          <n-scrollbar style="max-height: 400px;" v-if="importPreview">
            <n-descriptions bordered :column="1" label-placement="left">
              <n-descriptions-item label="Import Type">
                <n-tag>{{ importPreview.type }}</n-tag>
              </n-descriptions-item>
              <n-descriptions-item label="Items Count" v-if="importPreview.itemCount">
...
{{ JSON.stringify(importPreview.data, null, 2) }}
        ...
        <n-space justify="end" style="margin-top: 20px;">
          ...
          <n-button type="primary" @click="handleImport" :disabled="!importPreview">
            Import
          </n-button>
        </n-space>
      </div>
...
import { NModal, NSpace, NButton, NTabs, NTabPane, NSteps, NStep, NDescriptions, NDescriptionsItem, NTag, NScrollbar, NText, NIcon, useMessage } from 'naive-ui'
...
const handleNextStep = async () => {
  try {
    if (currentImportType.value === 'file' && selectedFile.value) {
      ...
      // 解析文本（cURL）
      const parsed = parseCurlCommand(textContent.value)
      if (!parsed) {
...
const handleImport = async () => {
  ...
  try {
    if (currentImportType.value === 'file' && selectedFile.value) {
      ...
      // cURL导入
      const parsed = importPreview.value.data
